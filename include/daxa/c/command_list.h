...
void daxa_cmd_copy_buffer_to_buffer(BufferCopyInfo const * info);
void daxa_cmd_copy_buffer_to_image(BufferImageCopyInfo const * info);
void daxa_cmd_copy_image_to_buffer(ImageBufferCopyInfo const * info);
void daxa_cmd_copy_image_to_image(ImageCopyInfo const * info);
void daxa_cmd_blit_image_to_image(daxa_CommandList cmd_list, ImageBlitInfo const * info);

void daxa_cmd_clear_buffer(daxa_CommandList cmd_list, BufferClearInfo const * info);
void daxa_cmd_clear_image(daxa_CommandList cmd_list, ImageClearInfo const * info);
...
void daxa_cmd_pipeline_barrier(daxa_CommandList cmd_list, MemoryBarrierInfo const * info);
/// @brief  Successive pipeline barrier calls are combined.
///         As soon as a non-pipeline barrier command is recorded, the currently recorded barriers are flushed with a vkCmdPipelineBarrier2 call.
/// @param info parameters.
void daxa_cmd_pipeline_barrier_image_transition(daxa_CommandList cmd_list, ImageBarrierInfo const * info);
void daxa_cmd_signal_event(daxa_CommandList cmd_list, EventSignalInfo const * info);
void daxa_cmd_wait_events(daxa_CommandList cmd_list, std::span<EventWaitInfo const> const * infos);
void daxa_cmd_wait_event(daxa_CommandList cmd_list, EventWaitInfo const * info);
void daxa_cmd_reset_event(daxa_CommandList cmd_list, ResetEventInfo const * info);

void daxa_cmd_push_constant_vptr(daxa_CommandList cmd_list, void const * data, uint32_t size, uint32_t offset = 0);
...
/// @param info parameters.
void daxa_cmd_set_uniform_buffer(daxa_CommandList cmd_list, SetConstantBufferInfo const * info);
void daxa_cmd_set_pipeline(daxa_CommandList cmd_list, ComputePipeline const * pipeline);
void daxa_cmd_set_pipeline(daxa_CommandList cmd_list, RasterPipeline const * pipeline);
void daxa_cmd_dispatch(daxa_CommandList cmd_list, uint32_t group_x, uint32_t group_y = 1, uint32_t group_z = 1);
void daxa_cmd_dispatch_indirect(daxa_CommandList cmd_list, DispatchIndirectInfo const * info);
...
void daxa_cmd_destroy_sampler_deferred(daxa_CommandList cmd_list, SamplerId id);
...
/// @param info parameters.
void daxa_cmd_begin_renderpass(daxa_CommandList cmd_list, RenderPassBeginInfo const * info);
/// @brief  Starts a renderpass scope akin to the dynamic rendering feature in vulkan.
///         Between the begin and end renderpass commands, the renderpass persists and drawcalls can be recorded.
void daxa_cmd_end_renderpass(daxa_CommandList cmd_list, );
void daxa_cmd_set_viewport(daxa_CommandList cmd_list, ViewportInfo const * info);
void daxa_cmd_set_scissor(daxa_CommandList cmd_list, Rect2D const * info);
void daxa_cmd_set_depth_bias(daxa_CommandList cmd_list, DepthBiasInfo const * info);
void daxa_cmd_set_index_buffer(daxa_CommandList cmd_list, daxa_BufferId id, size_t offset, size_t index_type_byte_size = sizeof(uint32_t));

void daxa_cmd_draw(daxa_CommandList cmd_list, DrawInfo const * info);
void daxa_cmd_draw_indexed(daxa_CommandList cmd_list, DrawIndexedInfo const * info);
void daxa_cmd_draw_indirect(daxa_CommandList cmd_list, DrawIndirectInfo const * info);
void daxa_cmd_draw_indirect_count(daxa_CommandList cmd_list, DrawIndirectCountInfo const * info);
void daxa_cmd_draw_mesh_tasks(daxa_CommandList cmd_list, uint32_t x, uint32_t y, uint32_t z);
void daxa_cmd_draw_mesh_tasks_indirect(daxa_CommandList cmd_list, DrawMeshTasksIndirectInfo const * info);
void daxa_cmd_draw_mesh_tasks_indirect_count(daxa_CommandList cmd_list, DrawMeshTasksIndirectCountInfo const * info);

void daxa_cmd_write_timestamp(daxa_CommandList cmd_list, WriteTimestampInfo const * info);
void daxa_cmd_reset_timestamps(daxa_CommandList cmd_list, ResetTimestampsInfo const * info);

void daxa_cmd_begin_label(daxa_CommandList cmd_list, CommandLabelInfo const * info);
void daxa_cmd_end_label(daxa_CommandList cmd_list, );

void daxa_cmd_complete(daxa_CommandList cmd_list, );
auto daxa_cmd_is_complete(daxa_CommandList cmd_list, ) const -> DAXA_BOOL;

auto daxa_cmd_info(daxa_CommandList cmd_list, ) const -> CommandListInfo const *;
