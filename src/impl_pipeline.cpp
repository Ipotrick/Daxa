...
{
    auto const & impl = *as<daxa_ImplRasterPipeline>();
    return *reinterpret_cast<RasterPipelineInfo const *>(&impl.info);
}
...
{
    auto const & impl = *as<daxa_ImplComputePipeline>();
    return *reinterpret_cast<ComputePipelineInfo const *>(&impl.info);
}

daxa_ImplRasterPipeline::daxa_ImplRasterPipeline(daxa_Device a_device, daxa_RasterPipelineInfo a_info)
    : ImplPipeline(a_device), info{a_info}
{
    // TODO(capi) Store info.name in std string!
...
daxa_ImplComputePipeline::daxa_ImplComputePipeline(daxa_Device a_device, daxa_ComputePipelineInfo a_info)
    : ImplPipeline(a_device), info{a_info}
{
// TODO(capi): implement!
#if 0
