...
auto create_surface(daxa_Instance instance, daxa_NativeWindowHandle handle, [[maybe_unused]] daxa_NativeWindowPlatform platform, VkSurfaceKHR * out_surface) -> daxa_Result
{
    ...
    VkResult vk_result = glfwCreateWindowSurface(instance->vk_instance, reinterpret_cast<GLFWwindow *>(handle), nullptr, out_surface);
    return std::bit_cast<daxa_Result>(vk_result);
#endif
}
...
#if defined(__APPLE__)
    return __c11_atomic_fetch_add((_Atomic(uint64_t) *)&mut_strong_ref, 1, static_cast<int>(std::memory_order::relaxed));
#else
...
auto ImplHandle::dec_refcnt(void (*zero_ref_callback)(ImplHandle const *), daxa_Instance instance) const -> u64
{
    auto & mut_strong_ref = *rc_cast<u64 *>(&this->strong_count);
#if defined(__APPLE__)
    auto prev = __c11_atomic_fetch_sub((_Atomic(uint64_t) *)&mut_strong_ref, 1, static_cast<int>(std::memory_order::relaxed));
#else
...
{
#if defined(__APPLE__)
    return __c11_atomic_load((_Atomic(uint64_t) *)&this->strong_count, static_cast<int>(std::memory_order::relaxed));
#else
...
#if defined(__APPLE__)
    return __c11_atomic_fetch_add((_Atomic(uint64_t) *)&mut_weak_ref, 1, static_cast<int>(std::memory_order::relaxed));
#else
...
auto ImplHandle::impl_dec_weak_refcnt(void (*zero_ref_callback)(ImplHandle const *), daxa_Instance, [[maybe_unused]] char const * callsite) const -> u64
{
    ...
    auto & mut_weak_ref = *rc_cast<u64 *>(&this->weak_count);
#if defined(__APPLE__)
    auto prev = __c11_atomic_fetch_sub((_Atomic(uint64_t) *)&mut_weak_ref, 1, static_cast<int>(std::memory_order::relaxed));
#else
...
{
#if defined(__APPLE__)
    return __c11_atomic_load((_Atomic(uint64_t) *)&this->weak_count, static_cast<int>(std::memory_order::relaxed));
#else
