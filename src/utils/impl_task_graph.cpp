...
namespace daxa
{
    ...
    auto static constexpr access_to_usage(TaskImageAccess const & access) -> ImageUsageFlags
    {
        switch (access)
        {
        case TaskImageAccess::SHADER_SAMPLED: [[fallthrough]];
        ...
        case TaskImageAccess::SHADER_STORAGE_READ_ONLY: [[fallthrough]];
        ...
        case TaskImageAccess::SHADER_STORAGE_READ_WRITE: [[fallthrough]];
        ...
        case TaskImageAccess::COMPUTE_SHADER_STORAGE_READ_WRITE:
    ...
    auto task_image_access_to_layout_access(TaskImageAccess const & access) -> std::tuple<ImageLayout, Access>
    {
        ...
        case TaskImageAccess::SHADER_SAMPLED: return {ImageLayout::READ_ONLY_OPTIMAL, {PipelineStageFlagBits::ALL_GRAPHICS | PipelineStageFlagBits::COMPUTE_SHADER, AccessTypeFlagBits::READ}};
        ...
        case TaskImageAccess::SHADER_STORAGE_READ_ONLY: return {ImageLayout::GENERAL, {PipelineStageFlagBits::PipelineStageFlagBits::ALL_GRAPHICS | PipelineStageFlagBits::COMPUTE_SHADER, AccessTypeFlagBits::READ}};
        ...
        case TaskImageAccess::SHADER_STORAGE_READ_WRITE: return {ImageLayout::GENERAL, {PipelineStageFlagBits::PipelineStageFlagBits::ALL_GRAPHICS | PipelineStageFlagBits::COMPUTE_SHADER, AccessTypeFlagBits::READ_WRITE}};
    ...
    auto to_string(TaskImageAccess const & usage) -> std::string_view
    {
        switch (usage)
        {
        case TaskImageAccess::SHADER_SAMPLED: return std::string_view{"SHADER_READ"};
        case TaskImageAccess::VERTEX_SHADER_SAMPLED: return std::string_view{"VERTEX_SHADER_READ"};
        case TaskImageAccess::TESSELLATION_CONTROL_SHADER_SAMPLED: return std::string_view{"TESSELLATION_CONTROL_SHADER_READ"};
        ...
        case TaskImageAccess::SHADER_STORAGE_WRITE_ONLY: return std::string_view{"SHADER_STORAGE_WRITE"};
        case TaskImageAccess::VERTEX_SHADER_STORAGE_WRITE_ONLY: return std::string_view{"VERTEX_SHADER_STORAGE_WRITE"};
        case TaskImageAccess::TESSELLATION_CONTROL_SHADER_STORAGE_WRITE_ONLY: return std::string_view{"TESSELLATION_CONTROL_SHADER_STORAGE_WRITE"};
        case TaskImageAccess::TESSELLATION_EVALUATION_SHADER_STORAGE_WRITE_ONLY: return std::string_view{"TESSELLATION_EVALUATION_SHADER_STORAGE_WRITE"};
        case TaskImageAccess::GEOMETRY_SHADER_STORAGE_WRITE_ONLY: return std::string_view{"GEOMETRY_SHADER_STORAGE_WRITE"};
        case TaskImageAccess::FRAGMENT_SHADER_STORAGE_WRITE_ONLY: return std::string_view{"FRAGMENT_SHADER_STORAGE_WRITE"};
        case TaskImageAccess::COMPUTE_SHADER_STORAGE_WRITE_ONLY: return std::string_view{"COMPUTE_SHADER_STORAGE_WRITE"};
        case TaskImageAccess::SHADER_STORAGE_READ_ONLY: return std::string_view{"SHADER_STORAGE_READ"};
        case TaskImageAccess::VERTEX_SHADER_STORAGE_READ_ONLY: return std::string_view{"VERTEX_SHADER_STORAGE_READ"};
        case TaskImageAccess::TESSELLATION_CONTROL_SHADER_STORAGE_READ_ONLY: return std::string_view{"TESSELLATION_CONTROL_SHADER_STORAGE_READ"};
        case TaskImageAccess::TESSELLATION_EVALUATION_SHADER_STORAGE_READ_ONLY: return std::string_view{"TESSELLATION_EVALUATION_SHADER_STORAGE_READ"};
        case TaskImageAccess::GEOMETRY_SHADER_STORAGE_READ_ONLY: return std::string_view{"GEOMETRY_SHADER_STORAGE_READ"};
        case TaskImageAccess::FRAGMENT_SHADER_STORAGE_READ_ONLY: return std::string_view{"FRAGMENT_SHADER_STORAGE_READ"};
        case TaskImageAccess::COMPUTE_SHADER_STORAGE_READ_ONLY: return std::string_view{"COMPUTE_SHADER_STORAGE_READ"};
        case TaskImageAccess::SHADER_STORAGE_READ_WRITE: return std::string_view{"SHADER_READ_WRITE"};
        case TaskImageAccess::VERTEX_SHADER_STORAGE_READ_WRITE: return std::string_view{"VERTEX_SHADER_READ_WRITE"};
        case TaskImageAccess::TESSELLATION_CONTROL_SHADER_STORAGE_READ_WRITE: return std::string_view{"TESSELLATION_CONTROL_SHADER_READ_WRITE"};
        case TaskImageAccess::TESSELLATION_EVALUATION_SHADER_STORAGE_READ_WRITE: return std::string_view{"TESSELLATION_EVALUATION_SHADER_READ_WRITE"};
        case TaskImageAccess::GEOMETRY_SHADER_STORAGE_READ_WRITE: return std::string_view{"GEOMETRY_SHADER_READ_WRITE"};
        case TaskImageAccess::FRAGMENT_SHADER_STORAGE_READ_WRITE: return std::string_view{"FRAGMENT_SHADER_READ_WRITE"};
        case TaskImageAccess::COMPUTE_SHADER_STORAGE_READ_WRITE: return std::string_view{"COMPUTE_SHADER_READ_WRITE"};
    ...
    auto TaskInterfaceUses::operator[](TaskBufferView const & handle) const -> TaskBufferUse<> const &
    {
        auto & impl = *static_cast<ImplTaskRuntimeInterface *>(this->backend);
        auto const local_handle = impl.task_graph.id_to_local_id(handle);
    ...
    auto TaskInterfaceUses::operator[](TaskImageView const & handle) const -> TaskImageUse<> const &
    {
        auto & impl = *static_cast<ImplTaskRuntimeInterface *>(this->backend);
        auto const local_handle = impl.task_graph.id_to_local_id(handle);
        auto iter = std::find_if(impl.current_task->base_task->get_generic_uses().begin(),
    ...
    {
        auto & impl = *static_cast<ImplTaskRuntimeInterface *>(this->backend);
        DAXA_DBG_ASSERT_TRUE_M(impl.task_graph.staging_memory.has_value(), "staging memory allocator is only available when transient memory pool size is greater then 0");
        return impl.task_graph.staging_memory.value();
    }
    ...
    TaskGraph::TaskGraph(TaskGraphInfo const & info)
        : ManagedPtr{new ImplTaskGraph(info)}
    {
    ...
    void validate_image_uses(ImplTaskGraph & impl, TaskGraphPermutation const & perm, u32 use_index, u32 task_image_index, TaskImageAccess task_access, std::string_view task_name)
    {
    ...
    void ImplTaskGraph::update_image_view_cache(ImplTask & task, TaskGraphPermutation const & permutation)
    {
        for_each(
            ...
            [&](u32 task_image_use_index, TaskImageUse<> & image_use)
            {
                ...
                // The image id here is alreadt the task graph local id.
    ...
    void validate_runtime_resources(ImplTaskGraph const & impl, TaskGraphPermutation const & permutation)
    {
#if DAXA_VALIDATION
    ...
    void ImplTaskGraph::check_for_overlapping_use(BaseTask & task)
    {
        for_each(
    ...
    void translate_persistent_ids(ImplTaskGraph const & impl, BaseTask & task)
    {
        for_each(
            task.get_generic_uses(),
    ...
    void TaskGraph::add_task(std::unique_ptr<BaseTask> && base_task)
    {
        ...
        // But in order to make the task graph implementation simpler,
    ...
    using ShaderUseIdOffsetTable = std::vector<std::variant<std::pair<TaskImageView, usize>, std::pair<TaskBufferView, usize>, std::monostate>>;
        ...
        BaseTask & task)
    {
        ...
        for_each(
            ...
            [&](u32, TaskBufferUse<> & arg)
            {
                if (task_graph_impl.global_buffer_infos[arg.handle.index].is_persistent())
                {
            ...
            {
                if (task_graph_impl.global_image_infos[arg.handle.index].is_persistent())
                {
        ...
        for_each(
            ...
            [&](u32, TaskBufferUse<> const & buffer_use)
            {
                ...
                if (!task_graph_impl.global_buffer_infos.at(buffer_use.handle.index).is_persistent())
                {
        ...
        CommandList & cmd_list)
    {
        ...
        for (usize task_buffer_index = 0; task_buffer_index < permutation.buffer_infos.size(); ++task_buffer_index)
        {
            ...
            if (task_buffer.valid && glob_buffer_info.is_persistent())
            {
                ...
                //      AS WE CANT MODIFY BARRIES FROM PREVIOUSLY ALREADY EXECUTED TASK GRAPHS, WE MUST OVERSYNC ON THE LAST WRITE TO READ
    ...
    void TaskGraph::execute(ExecutionInfo const & info)
    {
        ...
        ImplTaskRuntimeInterface impl_runtime{.task_graph = impl, .permutation = permutation};
