...
namespace daxa {
	...
	void BufferBackend::create(VkDevice device, VmaAllocator allocator, u32 queueIndex, BufferInfo const& info) {
		this->info = info;
		...
		DAXA_CHECK_VK_RESULT_M(vmaCreateBuffer(allocator, (VkBufferCreateInfo*)&bci, &aci, &(this->buffer), &(this->allocation), nullptr), "failed to create buffer");

		if (instance->pfnSetDebugUtilsObjectNameEXT != nullptr && info.debugName != nullptr) {
	...
	void BufferBackend::destroy(VmaAllocator allocator){
...
//
	...
	void* mapBuffer(VmaAllocator allocator, BufferBackend& buffer) {
		void* ret = nullptr;
		DAXA_CHECK_VK_RESULT_M(vmaMapMemory(allocator, buffer.allocation, &ret), "failed to map buffer memory");
		return ret;
	}

	void unmapBuffer(VmaAllocator allocator, BufferBackend& buffer) {
