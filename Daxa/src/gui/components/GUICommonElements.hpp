...
namespace daxa {
	namespace gui {
		...
		struct Checkbox : IElement {
			...
			Vec2 size{ Vec2{15,15} };
			Vec4 color{ UNSET_COLOR };
			Vec4 colorEnabled{ UNSET_COLOR };
			Vec4 colorDisabled{ UNSET_COLOR };
		};
		namespace {
		...
		struct SliderF64 : IElement {
			...
			f64* value{ nullptr };
			Vec2 size{ Vec2{150,25} };
			f64 min{ 0.0 };
			f64 max{ 1.0 };
			bool bVertical{ false };
			bool bThin{ true };
			Vec4 colorBar{ UNSET_COLOR };
			Vec4 colorSlider{ UNSET_COLOR };
			Vec4 colorError{ UNSET_COLOR };
			u32 child{ INVALID_ELEMENT_ID };
		};
