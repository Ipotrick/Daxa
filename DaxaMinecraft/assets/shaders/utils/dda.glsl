...
DDA_StartResult run_dda_start(in Ray ray, in out DDA_RunState run_state) {
    ...
    if (ray.nrm.x < 0) {
        ...
        result.ray_step.x = 1;
        run_state.to_side_dist_xy = (run_state.tile_i.x + 1.0f - ray.o.x) * result.delta_dist.x;
        run_state.to_side_dist_xz = (run_state.tile_i.x + 1.0f - ray.o.x) * result.delta_dist.x;
    }
    if (ray.nrm.y < 0) {
        result.ray_step.y = -1;
        ...
        result.ray_step.y = 1;
        run_state.to_side_dist_yx = (run_state.tile_i.y + 1.0f - ray.o.y) * result.delta_dist.y;
        run_state.to_side_dist_yz = (run_state.tile_i.y + 1.0f - ray.o.y) * result.delta_dist.y;
    }
    if (ray.nrm.z < 0) {
        result.ray_step.z = -1;
        ...
        result.ray_step.z = 1;
        run_state.to_side_dist_zx = (run_state.tile_i.z + 1.0f - ray.o.z) * result.delta_dist.z;
        run_state.to_side_dist_zy = (run_state.tile_i.z + 1.0f - ray.o.z) * result.delta_dist.z;
    }
    return result;
}
